...
class Proposicao:
    ...
    is_prl: bool = False
    praca: str = ""
    data_apresentacao: datetime | None = None
    ementa: str = ""
    orientacao: int = 0
    ...
    @property
    def nome(self):
        return self.sigla_tipo + " " + str(self.numero) + "/" + str(self.ano)
...
class ProposicaoAutor:
    ...
    def __post_init__(self):
        if isinstance(self.id_prop, int):
            self.id_prop = str(int(self.id_prop))
        if isinstance(self.id_autor, int):
            self.id_autor = str(int(self.id_autor))
...
class ArvoreApensados:
    ...
    def montar_graph(self, nodes: list[Node]):
        for node in nodes:
